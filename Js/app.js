...
function shApp() {
  ...
  // ============================
  // Real-time Permissions Listener (non-admin)
  // ============================
  if (!a && cu && cu.id) {
    ...
    unsPm = db.collection('users').doc(cu.id).onSnapshot(function(doc) {
      ...
      var d = doc.data();
      // If admin revoked approval, force logout
      if (d.approved === false) {
        setTimeout(function() {
          doOut();
      ...
      // Update permissions instantly without re-login
      var newPm = d.permissions || { canAdd:true, canDelete:true, canEdit:true };
      cu.permissions = newPm;
      sessionStorage.setItem('cu', JSON.stringify(cu));
      // Refresh add button visibility
      document.getElementById('addBtn').style.display = gP().canAdd ? 'inline-flex' : 'none';
      // Re-render table to reflect delete/edit permission changes
      rnT();
    }, function() {});
  }
}
...
function rnT() {
  ...
  // ============================
  // Build phone duplicate map (across ALL records)
  // ============================
  var dupPhones = {};
  ...
  tb.innerHTML = fl.map(function(c, i) {
    ...
    // Phone cell: highlight duplicates with red badge
    var phCell = isDup
...
function prsTm() {
  ...
  t.split('\n').forEach(function(l) {
    ...
    // ↓ سحب مبلغ الدفع / المبلغ المدفوع من القالب
  ...
  // مبلغ الدفع يتغلب على الرقم المستخرج من اسم الباقة
  if (pamt) {
